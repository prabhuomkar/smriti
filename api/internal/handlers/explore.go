...
func (h *Handler) GetYearsAgoMediaItems(ctx echo.Context) error {
	...
	if result.Error != nil {
		slog.Error("error getting years ago mediaitems", "error", result.Error)
...
func (h *Handler) GetPlaces(ctx echo.Context) error {
	...
	result := h.DB.Model(&models.Place{}).
		...
		Limit(limit)
	if result.Error != nil {
		slog.Error("error getting places", "error", result.Error)
		return echo.NewHTTPError(http.StatusInternalServerError, result.Error.Error())
	}
...
func (h *Handler) GetPlace(ctx echo.Context) error {
	...
	result := h.DB.Model(&models.Place{}).
		...
		First(&place)
	if result.Error != nil {
		slog.Error("error getting place", "error", result.Error)
...
func (h *Handler) GetThings(ctx echo.Context) error {
	...
	result := h.DB.Model(&models.Thing{}).
		...
		Limit(limit)
	if result.Error != nil {
		slog.Error("error getting things", "error", result.Error)
		return echo.NewHTTPError(http.StatusInternalServerError, result.Error.Error())
	}
...
func (h *Handler) GetThing(ctx echo.Context) error {
	...
	result := h.DB.Model(&models.Thing{}).
		...
		First(&thing)
	if result.Error != nil {
		slog.Error("error getting thing", "error", result.Error)
		if errors.Is(result.Error, gorm.ErrRecordNotFound) {
...
func (h *Handler) UpdatePerson(ctx echo.Context) error {
	...
	if result.Error != nil {
		slog.Error("error updating people", "error", result.Error)
		return echo.NewHTTPError(http.StatusInternalServerError, result.Error.Error())
	}
...
func (h *Handler) GetPeople(ctx echo.Context) error {
	...
	result := h.DB.Model(&models.People{}).
		...
		Limit(limit)
	if result.Error != nil {
		slog.Error("error getting people", "error", result.Error)
		return echo.NewHTTPError(http.StatusInternalServerError, result.Error.Error())
	}
...
func (h *Handler) GetPerson(ctx echo.Context) error {
	...
	result := h.DB.Model(&models.People{}).
		...
		First(&person)
	if result.Error != nil {
		slog.Error("error getting person", "error", result.Error)
		if errors.Is(result.Error, gorm.ErrRecordNotFound) {
...
func (h *Handler) GetPeopleMediaItems(ctx echo.Context) error {
	...
	if err != nil {
		slog.Error("error getting people mediaitems", "error", err)
		return echo.NewHTTPError(http.StatusInternalServerError, err.Error())
	}
	return ctx.JSON(http.StatusOK, mediaItems)
}

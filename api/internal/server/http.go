...
func StartHTTPServer(handler *handlers.Handler) *http.Server {
	...
	httpServer := &http.Server{
		Addr: fmt.Sprintf(
			"%s:%d",
			handler.Config.API.Host,
			handler.Config.API.Port,
		),
		Handler: srvHandler,
	}
	// metrics
	...
	hidden.Use(
		getMiddlewareFuncs(handler.Config, handler.Cache, true, "hidden")...)
	...
	trash.Use(
		getMiddlewareFuncs(handler.Config, handler.Cache, true, "trash")...)
	...
	explore.Use(
		getMiddlewareFuncs(handler.Config, handler.Cache, true, "explore")...)
	explore.GET(
		"/yearsAgo/:monthDate/mediaItems",
		handler.GetYearsAgoMediaItems,
	)
	...
	sharing.Use(
		getMiddlewareFuncs(handler.Config, handler.Cache, false, "sharing")...)

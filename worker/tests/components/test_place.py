...
from src.components.place import API_URL, process
...
def test_place_success(requests_mock):
    ...
    place = process(19.2195856, 73.1056888)
    assert place == {'postcode': '421201', 'country': 'India', 'state': 'Maharashtra', 'city': 'Dombivali', 'town': None}

def test_place_failed_empty_response(requests_mock):
    requests_mock.get(url=API_URL.format(lat=19.2195856, lon=73.1056888), json={})
    place = process(19.2195856, 73.1056888)
    assert place == {'postcode': None, 'country': None, 'state': None, 'city': None, 'town': None}
